...
class Agent(nn.Module):
    ...
    def update_memory(self, action_idx: torch.Tensor, reward: torch.Tensor):
        self.network.update_memory(action_idx, reward)
    ...
    def update(self, buffer: PPO_Buffer, last_value: torch.Tensor, batch_size: int = 32, shuffle=True):
        ...
        for _ in range(buffer.ppo_training_steps):
            for batch in buffer.sample_minibatches(batch_size=batch_size, shuffle=shuffle, device=self.device):
                ...
                new_log_policy = torch.log(new_policy.gather(1, actions.unsqueeze(1)))
        ...
        avg_critic_loss /= buffer.ppo_training_steps * len(buffer.states)
        avg_actor_loss /= buffer.ppo_training_steps * len(buffer.states)
        avg_entropy /= buffer.ppo_training_steps * len(buffer.states)
